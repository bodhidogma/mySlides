...
BOOL checkPassword(HWND hwnd){
	...
	if(osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS){  // this is Win95 or Win98
		HINSTANCE passwordcpl = LoadLibrary(_T("PASSWORD.CPL"));
		if(passwordcpl == NULL)
			return TRUE;
...
void changePassword(HWND hwnd){
	...
	GetVersionEx(&osvi);
	if(osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS){  // this is Win95 or Win98
		HINSTANCE mprdll = LoadLibrary(_T("MPR.DLL"));
		if(mprdll == NULL)
			return;
...
static int startScreenSaver(HWND parent){
	LPCTSTR windowClass = _T("WindowsScreenSaverClass");
	LPCTSTR windowTitle;
	WNDCLASS wclass;
	MSG msg;
	UINT style;
	...
	if(parent){
		...
		exStyle = 0;
		childPreview = 1;
		windowTitle = _T("Screensaver Preview");  // must differ from full screen window name
	}
	else{
...
outfile.close();*/
		...
		windowTitle = _T("Screensaver");  // must differ from preview window name
...
// convert _T to unsigned int
static UINT atoui(LPCTSTR str){
	UINT returnval = 0;
	while((*str >= _T('0')) && (*str <= _T('9')))
		returnval = ((returnval * 10) + (*str++ - _T('0')));
	return returnval;
}
...
static int startWindowedSaver(){
	...
	LPCTSTR windowClass = _T("WindowsScreenSaverClass");
	LPCTSTR windowTitle = _T("Windowed ScreenSaver");
	WNDCLASSEX wclass = {0};
	MSG msg = {0};
	UINT style = WS_OVERLAPPEDWINDOW | WS_POPUP | WS_VISIBLE;
	UINT styleEx = WS_EX_APPWINDOW;

	windowedSaver = 1;

	wclass.cbSize = sizeof(WNDCLASSEX);
	wclass.style = CS_OWNDC | CS_VREDRAW | CS_HREDRAW;
	wclass.lpfnWndProc = (WNDPROC)realScreenSaverProc;
	wclass.cbClsExtra = 0;
	wclass.cbWndExtra = 0;
	wclass.hInstance = mainInstance;
	wclass.hIcon = LoadIcon(mainInstance,MAKEINTATOM(ID_APP));
	...
	{
		mainWindow = CreateWindowEx(styleEx,windowClass,windowTitle, style,
			0,0, DEF_WIN_WIDTH, DEF_WIN_HEIGHT,
			HWND_DESKTOP, NULL, mainInstance, NULL);
	}
	if(mainWindow){
		while(1){
			// Message loop
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prevInst, LPSTR cmdLine, int cmdShow){
	LPCTSTR commandLine = (LPCTSTR)cmdLine;
	HWND parent = 0;
	...
	while(1){
		switch(*commandLine){
		case _T(' '):  // skip over whitespace
		case _T('-'):
		case _T('/'):
			commandLine++;
			break;
		case _T('a'):  // change password on Win95
		case _T('A'):
			changePassword(parent);
			return -1;
		case _T('c'):  // open "settings" dialog box
		case _T('C'):
			return openConfigBox(GetForegroundWindow());
		case _T('\0'):
			return openConfigBox(NULL);
		case _T('p'):  // run the preview
		case _T('P'):
			do
				commandLine++;
			while(*commandLine == _T(' '));  // skip over whitespace (I'm not sure why)
			return startSaverPreview(commandLine);
		case _T('s'):  // run the saver
		case _T('S'):
			return startScreenSaver(NULL);
		case _T('w'):  // run windowed saver, for debugging purposes
		case _T('W'):
...
#if 0
void initFrameRateLimitSlider(HWND hdlg, int sliderID, int _TID){
	...
	char cval[16];
	sprintf_s(cval, 16, "%d", dFrameRateLimit);
	SendDlgItemMessage(hdlg, _TID, WM_SET_T, 0, LPARAM(cval));
}

void updateFrameRateLimitSlider(HWND hdlg, int sliderID, int _TID){
	int ival = SendDlgItemMessage(hdlg, sliderID, TBM_GETPOS, 0, 0);
	char cval[16];
	sprintf_s(cval, 16, "%d", ival);
	SendDlgItemMessage(hdlg, _TID, WM_SET_T, 0, LPARAM(cval));
}
...
void readFrameRateLimitFromRegistry(){
	...
	if(result != ERROR_SUCCESS)
		return;
	valsize=sizeof(val);
	result = RegQueryValueEx(skey, _T("FrameRateLimit"), 0, &valtype, (LPBYTE)&val, &valsize);
	if(result == ERROR_SUCCESS)
		dFrameRateLimit = val;
